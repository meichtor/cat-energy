...
>>>>>>> effed153835467b2fbe6fca306c6c6cdc049c56f
      autoprefixer({
        browsers: [
          'last 2 version',
...
gulp.task('img', function () {
  return gulp.src(['app/img/*.{gif,png,jpg,jpeg,svg}'],
...
gulp.task('build', gulp.series(
  'clean',
  gulp.parallel('css', 'img', 'js', 'fonts'),
  'html'
));
...
gulp.task('png:sprite', function () {
  ...
  let cssStream = spriteData.css                            // запишем генерируемый стилевой файл спрайта
    .pipe(gulp.dest(dirs.source + '/sass/global'));
